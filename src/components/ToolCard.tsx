...
const ToolCard: React.FC<ToolCardProps> = (tool) => {
  ...
  return (
    <div
      ...
      className={`relative group w-full aspect-square rounded-2xl p-4 sm:p-6
    ...
    >
      ...
      <span className="text-white/90 font-medium text-sm sm:text-base relative z-10">{tool.name}</span>
      <p className="text-xs sm:text-sm text-white/50 text-center relative z-10">{tool.description}</p>
      
      {tool.comingSoon && (
        <div className="absolute top-2 sm:top-3 right-2 sm:right-3 px-2 py-1 bg-gradient-to-r from-purple-500/20 to-pink-500/20 rounded-full">
